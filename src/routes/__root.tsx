...
function ErrorComponent({ error, reset }: { error: Error; reset: () => void }) {
  ...
  // Auto-reload once on stale chunk errors (typical after a redeploy: old
  // index.html references chunks whose hashes no longer exist). A simple
  // router.invalidate() won't re-fetch the asset, only a full reload does.
  useEffect(() => {
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-background px-4">
        <div className="max-w-md text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">
            Detectamos una versión nueva. Recargando para aplicar los cambios.
          </p>
          <div className="mt-6">
            <button
              onClick={() => {
